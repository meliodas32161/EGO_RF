...
def _return_std(X, trees, predictions, min_variance=0.01):
    """
    Returns `std(Y | X)`.
...
def RandomForest_w(W,*args):
    ...
    x = W+x0
    x = x.reshape(1,-1)
    mu_ = model_RF.predict(x,return_std = False)
    return mu_
...
def integrand_mu(w,*args):
    ...
    # x     = args[2]
    # D = x.shape[0]
    return Gaussianprocess_w(w,model,x) * Normal(0.07).pdf(w,0)
...
def integrand_std(w,*args):
    ...
    # x     = args[2]
    # D = x.shape[0]
    return Gaussianprocess_w(w,model,x)**2 * Normal(0.07).pdf(w,0)
...
def integrand_std_G(w,*args):
    ...
    # x     = args[2]
    # D = x.shape[0]
    return Gaussianprocess_G(w,model,x)**2 * Normal(0.07).pdf(w,0)

def integrand_w_RF(w,*args):
    model,model_RF,x = args
    # model = args[1]
    # x     = args[2]
    # D = x.shape[0]
    return RandomForest_w(w,model,model_RF,x) * Normal(0.07).pdf(w,0)#p_w(w)
def integrand_std_w_RF(w,*args):
    model,model_RF,x = args
    # model = args[1]
    # x     = args[2]
    # D = x.shape[0]
    return RandomForest_w(w,model,model_RF,x)**2 * Normal(0.07).pdf(w,0)#p_w(w)
# def integrand_std_G_RF(w,*args):
#     model,model_RF,x = args
#     # model = args[1]
#     # x     = args[2]
#     # D = x.shape[0]
#     return RandomForest_G(w,model,model_RF,x)**2 * Normal(0.01).pdf(w,0)#p_w(w)
def integrand_std_G_RF(w,*args):
    model,model_RF,x = args
    # model = args[1]
    # x     = args[2]
    # D = x.shape[0]
    return RandomForest_G(w,model,model_RF,x)**2 * Normal(0.07).pdf(w,0)#p_w(w)
...
def convolute_RF(X,dists,model,model_RF,bound):
    ...
    if len(np.shape(X)) == 1:
        ...
        # result_mu,error_w = nquad(integrand_w_RF, [[X-bound[0], bound[1]-X]],args=(model,model_RF,X))
        # result_std,error_std = nquad(integrand_std_w_RF, [[X-bound[0], bound[1]-X]],args=(model,model_RF,X))
        # mu_wG.append(result_mu)
        # std_w.append(np.sqrt(result_std - result_mu**2))
        # std_w.append(result_std)
        
        # 考虑模型和参数不确定性的方差
        # result_std_G, error_G = nquad(integrand_std_G_RF, [[-np.inf, np.inf]],args=(model,model_RF,X))
        ...
        for x0 in X:
            ...
            # 考虑模型和参数不确定性的方差
            # result_std_G, error_G = nquad(integrand_std_G_RF, [[-np.inf, np.inf]],args=(model,model_RF,x0),opts={"limit":300})
...
def gaussian_ei(X,*args):
    ...
    if model_type == 'gp':
        ...
        with warnings.catch_warnings():
            ...
            mu,_,std = convolute_RF(X,dists=distribution,model=model,model_RF=model_RF,bound=bounds)
            if (mu.ndim != 1):
                mu = mu.flatten()
            if (std.ndim != 1):
                std = std.flatten()
...
# 定义采集函数取最大的函数
def propose_location(acquisition ,model,model_RF, X_sample, Y_sample, bounds,distributions=None,model_type='gp', n_restarts = 10):
...
def robust_optimization_GW(objective_function,bounds, n_samples, n_iterations,
    ...
    for i in range(n_iterations):
        ...
        if model_type == 'rf':
            ...
            y_pred_mean_3 = model_RF.predict(x_true_3)

            
            # RandomForest 推荐值
            x_next_3 = propose_location(gaussian_ei, model,model_RF,X_3, y_3, bounds,distributions = dists,model_type=model_type, n_restarts = 10)
            # y_next_mean_3 = model.forest.predict(x_next_3)
            # y_next_3 = objective_function(x_next_3.flatten())
            y_next_3 = np.array(objective_function(x_next_3.flatten()))
            X_3 = np.vstack([X_3, x_next_3])
            y_3 = np.vstack([y_3, y_next_3])
            # y_3.append(y_next_3)
            print(x_next_3)
            print(y_next_3)
            plt.figure()
    ...
    if model_type=='rf':
        ...
        y_pred_3,y_pred_std_3 = model_RF.predict(X_3,return_std=True)
        
        # y_pred_3 = model.forest.predict(X_3)
        # y_pred_std_3 = _return_std(X_3, model.forest, y_pred_3, min_variance=1e-6)
        _,std_w_3,std_WG_3 = convolute_RF(X_3,dists, model,model_RF,bounds)
        plt.scatter(X_3,y_pred_std_3)
        plt.scatter(X_3,std_w_3)
        plt.scatter(X_3,std_WG_3)
        # plt.scatter(X_3,y_pred_std_3[index])
    ...
    return mu_wG, std_WG,std_w,y_pred_mean_3, y_pred_std_3
...
def robust_optimization_GW_2(objective_function,bounds, n_samples, n_iterations,
    ...
    for i in range(n_iterations):
        ...
        # result_std_G, error_G = nquad(integrand_std, bounds,args=(model,x0))
    if model_type == 'gp':
        mu_wG,std_w,std_WG = convolute_RF(x_true_3,model,model_RF)
    elif model_type == 'rf':
        mu_wG,std_w,std_WG = convolute_RF(x_true_3,distributions,model,model_RF,bound)
...
# mu_wG_K,  std_WG_K,std_W_K,y_pred_mean_K, y_pred_std_K= robust_optimization_GW(onedimention_problem, bounds_1, n_samples, n_iterations,
#                                         D=1,samples=samples,model_type='gp') 
dists = [Normal(0.07)]
mu_wG_RF,  std_WG_RF,std_W_RF,y_pred_mean_RF, y_pred_std_RF = robust_optimization_GW(onedimention_problem, bounds_1,
