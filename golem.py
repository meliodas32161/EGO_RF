...
# import sys
# sys.path.append("C:\\Users\\WY229\\OneDrive\\文档\\machine learning\\input uncertainty\\my_programm")
import pyximport
pyximport.install()
...
class Golem(object):
    ...
    def predict(self, X, distributions, return_std=False, return_unc=False):
        ...
        # self.var_robust = np.mean(np.array(self._stds_robust)**2, axis=0)  # variance of the output, Var[f(X)]
        ...
        #         # E[f(X)], Var[f(X)]
        #         return self.y_robust, self.std_robust,self.var_robust
        ...
        self.y_robust_std = np.std(self._ys_robust, axis=0)  # Var[E[f(X)]]
        self.std_robust = np.mean(self._stds_robust, axis=0)  # variance of the output, Var[f(X)]
        self.var_robust = np.mean(np.array(self._stds_robust)**2+np.array(self._ys_robust)**2, axis=0)  # expectation of the output**2, E[f(X)**2] 我自己加的
        self.std_robust_std = np.std(self._stds_robust, axis=0)  # Var[Var[f(X)]]
    ...
    def _parse_fit_X(self, X):
        """ Parse input parameters X. This method should be called by ``fit`` only
            解析输入参数 X。该方法只能由 ``fit`` 调用
        """
        self._df_X = None  # initialize to None
        if isinstance(X, pd.DataFrame): # 如果X是由pandas导入，是PD.Dataframe类型
    ...
    def _parse_tree(self, tree):
        ...
        提取有关树结构的信息，并最终提取所有瓦片/端叶的边界和预测值
        """
...
class Golem_std(object):
    ...
    def _parse_tree(self, tree,min_variance=0.01):
        """Extract information about tree structure and eventually the bounds and predictions for all
        tiles/terminal leaves
        提取有关树结构的信息，并最终提取所有瓦片/端叶的边界和预测值
        """
        ...
        feature = tree.tree_.feature  # features split at nodes
        threshold = tree.tree_.threshold  # threshold used at nodes
        value_mu = tree.tree_.value  # model value of leaves
        
        #====================================== 子叶的方差 这是我额外加的===================================
        value_std = tree.tree_.impurity[tree.apply(self._X)]
        value_std[value_std < min_variance] = min_variance
        value = value_std + (1-1/len(self.forest.estimators_))*value_mu**2 
        # value = value_std + value_mu**2
...
class Golem_std_mu(object):
    ...
    def fit(self, X, y):
        ...
        if self._nproc > 1:
            ...
            # for i, tree in enumerate(self.forest.estimators_):
            #     if i < len(self.forest.estimators_)-1:
            #         _bounds, _pred = self._parse_tree(tree)
            #         _preds = 0
            #         i_ = len(self.forest.estimators_)-1-i
            #         if i_>0 :
            #             for j in range(i_):
            #                 _preds =_preds+ _pred*self._parse_tree(self.forest.estimators_[i+j])[-1]
            ...
            # for i, treei in enumerate(self.forest.estimators_):
            #     _bound, _pred = self._parse_tree(treei)
            #     _preds = sum(self._parse_tree(treej)[1] for j,treej in enumerate(self.forest.estimators_))
            #     self._preds.append(_preds)
            #     self._bounds.append(_bound)
            for i, tree in enumerate(self.forest.estimators_):
                _bounds, _preds = self._parse_tree(tree)
                self._bounds.append(_bounds)
                self._preds.append(_preds)
            ...
            #     # # 计算后续所有树的预测乘积之和
            #     # _preds = sum(_pred * self._parse_tree(self.forest.estimators_[j])[1]
            #     #              for j in range(i + 1, len(self.forest.estimators_)))
            #     _preds = sum(_pred * self._parse_tree(treej)[1] for j,treej in enumerate(self.forest.estimators_))
            #     self._preds.append(_preds)
            #     self._bounds.append(_bound)
        end = time.time()
        self.logger.log(f'{self._ntrees} tree(s) parsed in %.2f %s' % parse_time(start, end), 'INFO')
    ...
    def predict(self, X, distributions, return_std=False, return_unc=False):
        ...
        if self._nproc > 1:
            ...
            # for i, tree in enumerate(self.forest.estimators_):                
            #     y_robust, std_robust = convolute(_X, self._distributions, self._preds[i], self._bounds[i])
            #     self._ys_robust.append(y_robust/self.ntrees)
            #     self._stds_robust.append(std_robust)
            for i,pred in enumerate(self._preds):                
                y_robust, std_robust = convolute(_X, self._distributions, pred, self._bounds[i])
                self._ys_robust.append(y_robust)
                self._stds_robust.append(std_robust)
        ...
        # take the average across all trees
        # self.y_robust = np.mean(self._ys_robust, axis=0)*len(self._ys_robust)/len(self.forest.estimators_)  # expectation of the output, E[f(X)]
        self.y_robust = np.mean(self._ys_robust, axis=0)/self.ntrees**2
        # self.y_robust = np.array(self._ys_robust)/self.ntrees * 2
        self.y_robust_std = np.std(self._ys_robust, axis=0)  # Var[E[f(X)]]
        self.std_robust = np.mean(self._stds_robust, axis=0)  # variance of the output, Var[f(X)]
        self.std_robust_std = np.std(self._stds_robust, axis=0)  # Var[Var[f(X)]]
    ...
    def _parse_tree(self, tree,min_variance=0.01):
        ...
        for i, treei in enumerate(self.forest.estimators_):
            ...
            value = sum(treej.tree_.value for j,treej in enumerate(self.forest.estimators_))
        # value = value_mu 
...
class Golem_std_mu_2(object):
    ...
    def predict(self, X, distributions, return_std=False, return_unc=False):
        ...
        if self._nproc > 1:
            ...
            # perform convolution, use multiple processors when we have >1 trees
            with ProcessPoolExecutor(max_workers=self._nproc) as executor:
                for y_robust, std_robust in executor.map(convolute_2, *zip(*args)):
                    self._ys_robust.append(y_robust)
                    self._stds_robust.append(std_robust)
        else:
            for i, tree in enumerate(self.forest.estimators_):
